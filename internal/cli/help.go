...
var commandHelpRegistry = map[string]commandHelp{
	...
	"trace clear": {
		Path: "trace clear", Summary: "Explicitly clear the origin map of one existing Candidate.",
		Usage:   []string{"sealgraph trace clear REF [--format human|json]"},
		Options: []helpOption{{"--format human|json", "optional, once; default terminal=human, non-terminal=JSON"}},
		Details: []string{"Content, attachments, root, draft, Cause Links, and expected REF head remain unchanged. Clearing an already absent origin reports no change."},
		Related: []string{"trace set", "trace show", "candidate show"},
	},
	"trace show": {
		Path: "trace show", Summary: "Inspect stored origin records without reading current source files.",
		Usage:   []string{"sealgraph trace show (--ref REF | --seal SELECTOR) [--format human|json]"},
		Options: []helpOption{{"--ref REF", "select one logical REF with separate Candidate and HEAD baselines"}, {"--seal SELECTOR", "select one exact immutable Seal"}, {"--format human|json", "optional, once; default terminal=human, non-terminal=JSON"}},
	...
	"load": {
		Path: "load", Summary: "Atomically import one supported snapshot into an absent target.", Usage: []string{"sealgraph load --format universal-blob-v1 < repository.dump.json", "sealgraph load --format native-blobs-v1 --file PATH --max-input-bytes N"}, Arguments: []string{"Universal input is read from stdin. Native input is read from one named regular non-symlink file."}, Options: []helpOption{{"--format universal-blob-v1|native-blobs-v1", "required exactly once"}, {"--file PATH", "required for native-blobs-v1; named regular non-symlink input file"}, {"--max-input-bytes N", "required for native-blobs-v1; positive input limit"}}, Details: []string{"Universal-blob-v1 preserves the existing format-4 migration workflow and reads stdin. Native-blobs-v1 requires a format-7 snapshot, publishes only to an absent target, and never merges, replaces, or repairs existing state. If native receipt stdout delivery fails after publication, stderr reports LOAD_COMMITTED_OUTPUT_UNDELIVERED; never retry load."}, Examples: []string{"sealgraph load --format universal-blob-v1 < repository.dump.json", "sealgraph load --format native-blobs-v1 --file repository.snapshot.json --max-input-bytes N"}, Related: []string{"dump", "migrate extract", "init", "fsck"},
	},
	"dump": {
		Path: "dump", Summary: "Emit the complete retained format-7 native snapshot.", Usage: []string{"sealgraph dump --format native-blobs-v1"}, Arguments: []string{"No positional arguments."}, Options: []helpOption{{"--format native-blobs-v1", "required exactly once; format-7 only"}}, Details: []string{"Stdout is one canonical sealgraph/native-snapshot/v1 document. The snapshot includes every retained Blob, including opaque orphan Blobs. A notice is emitted on stderr after successful document delivery."}, Examples: []string{"sealgraph dump --format native-blobs-v1 > repository.snapshot.json"}, Related: []string{"load", "fsck"},
	},
	"load-receipt": {

# Candidate explicit Seal comparison requirement R3

Status: Requirement Candidate R3 — Awaiting First Owner Review
...
comparison is insufficient, followed by the selection of the explicit-option
form:
...
received a completed independent-review verdict of `FAIL` because its
revalidation wording narrowed ADR 0023's complete snapshot discipline. R2 at
SHA-256
`50b6b1b69869abdc3b761d35670bbecde3faf1c9fccaa0bb5197531e7bc16a27`
corrected that boundary and received a completed independent-review verdict of
`FAIL` because it omitted the accepted architecture comparison boundary from
its prior-authority disposition and synchronization criterion. The owner
selected `REVISE` for both snapshots.

R3 adds the architecture boundary and synchronization requirement. It does not
change the selected CLI, comparison semantics, schemas, snapshot discipline,
or scope.
...
- `docs/requirements.md` and `docs/cli.md` carry the same publication-baseline
  restriction.
- `docs/architecture.md` defines Candidate comparison against its explicit
  publication baseline as part of the history and comparison boundary.
...
1. The accepted ADR, requirements, architecture, CLI contract, help, and schema
   documentation describe the explicit mode without contradicting the
...
1. Does the candidate identify and synchronize every accepted
   publication-baseline-only restriction, including the architecture boundary,
   without superseding unrelated authority?
2. Does the v2-preservation/v3-explicit split avoid changing existing machine
   output under an old schema identifier?
3. Does R3 preserve ADR 0023's complete `O` recapture and equality contract and
...
- Source provenance states the R1 and R2 review results and owner `REVISE`
  routes and does not treat existing implementation as requirement authority.
- Every directly affected accepted restriction, including architecture, is
  identified; unrelated top-level, source, revision, mutation, and Git
  boundaries are preserved.
- R3 explicitly preserves complete `O` revalidation and conditional complete
  `I_O` revalidation.
...
- Acceptance criteria cover requirements, architecture, CLI, schemas,
  semantics, complete observation, errors, focused tests, and regression
  validation.
- Unknown future operand pairs remain non-blocking.
- The proposed independent-review input is bounded to this requirement.

## 10. Material differences from R2

- Records the completed R2 `FAIL` review and owner `REVISE` disposition.
- Adds the accepted architecture publication-baseline boundary to the explicit
  prior-authority disposition.
- Adds architecture to the documentation synchronization acceptance criterion.
- Updates the independent-review question and self-review accordingly.
- Makes no functional, CLI, schema, snapshot, or scope change.
